...
const getDiscountPercent = (productName) =>
  discounts[productName] / 100 || 0;

const getTaxPercent = (productName) =>
  taxes[productName] / 100 || 0;
...
const getSum = () => {
  const lineItemPriceList = purchases.map((purchase) =>
    getLineItemPrice(purchase));
  const total = lineItemPriceList.reduce((acc, currentvalue) =>
    acc + currentvalue.subTotal, 0);
  return { total, lineItemPriceList };
};

const main = () => {
  const { lineItemPriceList, total } = getSum();
  console.table(lineItemPriceList);
  console.log(total);
}
main();